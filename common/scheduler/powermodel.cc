...
int PowerModel::getExpectedGoodFrequency(int currentFrequency, float powerConsumption, float powerBudget, const std::vector<int> &Frequency){
	int expectedGoodFrequency = Frequency.at(0);
	for(int f = 0; f <= Frequency.size(); f++){
		float expectedPower = estimatePower(currentFrequency, powerConsumption, Frequency.at(f));
		if (expectedPower <= powerBudget) {
			expectedGoodFrequency = Frequency.at(f);
		}
	}

	if (expectedGoodFrequency > Frequency.at(Frequency.size()-1)) {
		expectedGoodFrequency = Frequency.at(Frequency.size()-1);
	}
	if (expectedGoodFrequency < Frequency.at(0)) {
		expectedGoodFrequency = Frequency.at(0);
	}
	return expectedGoodFrequency;
}
